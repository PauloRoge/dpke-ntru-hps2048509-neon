...
static void uint64N_to_uint8_array(const std::array<uint64_t, N> &poly, uint8_t v[64 * N]) {
    for (size_t i = 0; i < 64 * N; i++) v[i] = (poly[i / 64] >> (i % 64)) & 1;
}

static void ref_poly_mul(const uint8_t a[], const uint8_t b[], uint8_t c[], size_t ncoeffs) {
    std::memset(c, 0, (2 * ncoeffs - 1) * sizeof(uint8_t));

    for (size_t j = 0; j < ncoeffs; j++)
        for (size_t i = 0; i < ncoeffs; i++) c[i + j] ^= static_cast<uint8_t>(a[i] & b[j]);
}
...
static void uint8_array_to_uint64_2N(const uint8_t in[128 * N - 1], std::array<uint64_t, 2 * N> &out) {
    ...
    for (size_t i = 0; i < 128 * N - 1; i++) out[i / 64] |= (uint64_t(in[i] & 1) << (i % 64));
}
...
const binary_polynomial_multiplication_test_case_t<1> test_cases_64[] = {
    ...
    {{UINT64_C(0xc9f7d8ab7f2d1a29)},
     {UINT64_C(0xa3dace72b96811d5)},
...
const binary_polynomial_multiplication_test_case_t<2> test_cases_128[] = {
    ...
    {{UINT64_C(0x168c1c323e0ce3f9), UINT64_C(0x89548d484c8a6c7c)},
     ...
     {UINT64_C(0xd51a927bea83ba53), UINT64_C(0x15b7d2daeee743dc), UINT64_C(0xd5189e4d4a3a2bba),
      UINT64_C(0x51249ab14afe4904)}},
...
const binary_polynomial_multiplication_test_case_t<4> test_cases_256[] = {
    {{0x0, 0x0, 0x0, 0x0}, {0x0, 0x0, 0x0, 0x0}, {0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0}},  // 0 * 0 = 0

    {{0x1, 0x0, 0x0, 0x0}, {0x1, 0x0, 0x0, 0x0}, {0x1, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0}},  // 1 * 1 = 1

    {{0x0, 0x1, 0x0, 0x0}, {0x1, 0x0, 0x0, 0x0}, {0x0, 0x1, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0}},  // x^64 * 1 = x^64

    {{0x0, 0x0, 0x1, 0x0}, {0x1, 0x0, 0x0, 0x0}, {0x0, 0x0, 0x1, 0x0, 0x0, 0x0, 0x0, 0x0}},  // x^128 * 1 = x^128

    {{0x0, 0x0, 0x0, 0x1}, {0x1, 0x0, 0x0, 0x0}, {0x0, 0x0, 0x0, 0x1, 0x0, 0x0, 0x0, 0x0}},  // x^192 * 1 = x^192

    // random
    {{UINT64_C(0x7a16b2fe325764e7), UINT64_C(0xaf5a130c243f79e7), UINT64_C(0x9ca82c06b15263dd),
      UINT64_C(0xc8de8343f474e889)},
     {UINT64_C(0xa31d4a1841315e16), UINT64_C(0x9e71014ca372e0b6), UINT64_C(0x6d467d5e8cb24e20),
      UINT64_C(0x92317b5a01283f42)},
     {UINT64_C(0xa4e097b0fef8ce22), UINT64_C(0xd92755f774ebe1dc), UINT64_C(0x87ac0d6568c417f8),
      UINT64_C(0x41763e456d6ad35e), UINT64_C(0x966209aa7345a5b5), UINT64_C(0xfc357c6401a4510c),
      UINT64_C(0x9b8fbe52a4a79202), UINT64_C(0x69666af2ff52e3b5)}},
...
const binary_polynomial_multiplication_test_case_t<8> test_cases_512[] = {
    ...
    // random
    {{UINT64_C(0x123456789abcdef0), UINT64_C(0x0fedcba987654321), UINT64_C(0x55aa55aa55aa55aa),
     ...
     {UINT64_C(0xfedcba9876543210), UINT64_C(0x0123456789abcdef), UINT64_C(0xaa55aa55aa55aa55),
      UINT64_C(0x55aa55aa55aa55aa), UINT64_C(0x0fedcba987654321), UINT64_C(0x123456789abcdef0),
      UINT64_C(0xf0f0f0f0f0f0f0f0), UINT64_C(0x0f0f0f0f0f0f0f0f)},

     {UINT64_C(0x0a0789828c810f00), UINT64_C(0x0b4688c28dc00e44), UINT64_C(0x399a508895c9fcdf),
      UINT64_C(0x5d9e88b389b55c98), UINT64_C(0x44bffb7d6e6ad1a8), UINT64_C(0x20ab7347320621aa),
      UINT64_C(0x2b5c8748c60e2a5a), UINT64_C(0x1a1d5a27cf378f0d), UINT64_C(0x3a1390c111c7bb15),
      UINT64_C(0x0b421daf58ee4e07), UINT64_C(0x399ec5e540e7bc9c), UINT64_C(0x6d8a91f014f3e88d),
      UINT64_C(0x5bf2a40aa40d5bf5), UINT64_C(0x5bf2a40aa40d5bf5), UINT64_C(0x5005500550055005),
      UINT64_C(0x0550055005500550)}},
